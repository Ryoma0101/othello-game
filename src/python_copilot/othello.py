...
class OthelloCPU:
    ...
    def _evaluate_board(self, board: OthelloBoard) -> float:
        ...
        # Piece difference
        black_score, white_score = board.get_score()
        piece_diff = (black_score - white_score) * (1 if self.player == Player.BLACK else -1)
        score += piece_diff * 10
...
class OthelloGame:
    ...
    def play(self) -> None:
        ...
        print(f"You are playing as {'Black (B)' if self.human_player == Player.BLACK else 'White (W)'}")
        print(f"CPU is playing as {'White (W)' if self.cpu_player == Player.WHITE else 'Black (B)'}")
        ...
        while not self.board.is_game_over():
            ...
            if not valid_moves:
                print(f"No valid moves for {self.current_player.name}. Passing turn.")
                self._switch_player()
                continue
